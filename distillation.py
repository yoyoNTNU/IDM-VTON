...
def main():
    ...
    # ip-adapter
    image_proj_model = student.encoder_hid_proj.to(accelerator.device, dtype=torch.float32)
    ...
    params_to_opt = itertools.chain(student.parameters())
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
                            ...
                            with torch.cuda.amp.autocast():
                                ...
                                with torch.no_grad():
                                    for sample in test_dataloader:
                                        ...
                                        for i in range(len(images)):
                                            images[i].save(os.path.join(args.output_dir, str(global_step) + "_" + str(
                                                i) + "_" + "test_student.jpg"))
                                        break
                        del unwrapped_student
                        del newpipe
        ...
        if global_step % args.checkpointing_epoch == 0:
            if accelerator.is_main_process:
                ...
                unwrapped_student = accelerator.unwrap_model(
                    student, keep_fp32_wrapper=True
                )
                pipeline = TryonPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    unet=unwrapped_student,
                    vae=vae,
                    scheduler=noise_scheduler,
                    tokenizer=None,
                    tokenizer_2=None,
